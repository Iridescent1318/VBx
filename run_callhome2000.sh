...
mkdir -p callhome2000_exp

for audio in $(ls /home/liaozty20/callhome2000/wav)
do
      filename=$(echo "${audio}" | cut -f 1 -d '.')
      echo ${filename} > callhome2000_exp/list.txt
      echo "X-vectors Extraction Starts: "${filename}""
      # run feature and x-vectors extraction
      python VBx/predict.py --in-file-list callhome2000_exp/list.txt \
          --in-lab-dir /home/liaozty20/callhome2000/labs \
          --in-wav-dir /home/liaozty20/callhome2000/wav \
          --out-ark-fn callhome2000_exp/${filename}.ark \
          --out-seg-fn callhome2000_exp/${filename}.seg \
          --backend pytorch \
      ...
      python VBx/vbhmm.py --init AHC+VB \
          --out-rttm-dir callhome2000_exp \
          --xvec-ark-file callhome2000_exp/${filename}.ark \
          --segments-file callhome2000_exp/${filename}.seg \
      ...
      REFDIR=/home/liaozty20/callhome2000/rttm/${filename}.rttm
      SYSDIR=callhome2000_exp/${filename}.rttm
      if [ -f $REFDIR ]
      then
          # run dscore
          # forgiving
